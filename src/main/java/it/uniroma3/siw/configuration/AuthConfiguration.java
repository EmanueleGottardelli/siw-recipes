	...
	protected SecurityFilterChain configure(HttpSecurity http) throws Exception {
		http
				...
				.requestMatchers(HttpMethod.GET, "/", "/index", "/login", "/register", "/css/**", "/images/**",
						"/recipe", "/recipe/**", "/searchRecipes")
				.permitAll()
				// chiunque puo mandare richieste post a login e register
				.requestMatchers(HttpMethod.POST, "/login", "/register").permitAll()
				// solo gli utenti autenticati con ruolo ADMIN possono accedere al path
				// /admin/**
				...
				.and().formLogin().loginPage("/login").defaultSuccessUrl("/success", true)
